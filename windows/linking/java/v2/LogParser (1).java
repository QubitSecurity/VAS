...
public class LogParser {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            String targetKey = "targetFile"; // Specify the key you want to extract
            String result = parser.parse(inputLog, patterns, resolverConfigText, targetKey);
            if (result == null) {
    ...
    public String parse(String inputLog, String patterns, String resolverConfigText, String targetKey) throws IOException {
        ...
        return parse(inputLog, rules, targetKey);
    }
    ...
    public String parse(String inputLog, List<GrokRule> rules, String targetKey) {
        for (GrokRule rule : rules) {
            ...
            if (matcher.matches() || matcher.find()) {
                ...
                sb.append("{\n");
                sb.append("  \"category\": \"").append(escapeJson(rule.category)).append("\",\n");
                sb.append("  \"name\": \"").append(escapeJson(rule.name)).append("\",\n");
                sb.append("  \"values\": [");
                ...
                for (String groupName : rule.groupNames) {
                    ...
                    if (extractedValue != null) {
                        String resolvedValue = pathResolver.resolve(
                                groupName,
                                extractedValue,
                                inputLog);
                        if (!firstValue) sb.append(",");
                        sb.append("\n    {\n");
                        sb.append("      \"key\": \"").append(escapeJson(groupName)).append("\",\n");
                        sb.append("      \"value\": \"").append(escapeJson(resolvedValue)).append("\"\n");
                        sb.append("    }");
                        firstValue = false;
                    }
                }
                ...
                */
                ...
                // 2. targetFile 키 값만 출력
                for (String groupName : rule.groupNames) {
                    if (targetKey.equals(groupName)) {
                        String extractedValue = matcher.group(groupName);
                        if (extractedValue != null) {
                            String resolvedValue = pathResolver.resolve(
                                    groupName,
